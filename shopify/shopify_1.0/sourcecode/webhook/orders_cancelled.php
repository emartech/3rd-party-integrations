...
if($verified == 1){
	...
	$query = "SELECT * FROM `emarsys_event_mapper` WHERE `fkShopifyEventID` = '" . ORDER_CANCELLED . "' AND `store_name` = '" . trim($shop_name) . "'";
	$result = $con->query($query);
	if(DEV_ENV){
		$file = fopen('orders_cancelled_payload.txt', 'w');
	}
	if($result->num_rows > 0){
		...
		//-------- get the webhook data ------------
			...
			if(DEV_ENV){
				$sfd = fopen('shopify_orders_cancelled.txt', 'w');
			...
			if($result->num_rows > 0){
				...
				$query = "SELECT * FROM `emarsys_order_placeholders` WHERE `fkShopifyEventID` = ' " . ORDER_CANCELLED . "' AND `store_name` = '" . $shop_name . "'";
