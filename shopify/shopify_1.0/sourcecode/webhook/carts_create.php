...
if($verified == 1){
	...
	$query = "SELECT * FROM `emarsys_event_mapper` WHERE `fkShopifyEventID` = '" . CARTS_CREATE . "' AND `store_name` = '" . trim($shop_name) . "'";
	$result = $con->query($query);
	if(DEV_ENV){
		$file = fopen('cart_create_paylooad.txt', 'w');
	}
	if($result->num_rows > 0){
		...
		//-------- get the webhook data ------------
			...
			if(DEV_ENV){
				$sfd = fopen('shopify_cart_carete.txt', 'w');
			...
			if($result->num_rows > 0){
				...
				$query = "SELECT * FROM `emarsys_cart_placeholders` WHERE `fkShopifyEventID` = ' " . CARTS_CREATE . "' AND `store_name` = '" . $shop_name . "'";

				$result = $con->query($query);
				if($result->num_rows > 0){
					require_once 'payload/carts.php';
				}
			}
