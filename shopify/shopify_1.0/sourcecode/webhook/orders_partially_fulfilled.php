...
define('ORDER_PARTIALLY_FULFILLED', 19);
...
if($verified == 1){
	...
	$query = "SELECT * FROM `emarsys_event_mapper` WHERE `fkShopifyEventID` = '" . ORDER_PARTIALLY_FULFILLED . "' AND `store_name` = '" . trim($shop_name) . "'";
	$result = $con->query($query);
	if(DEV_ENV){
		$file = fopen('orders_partially_fulfilled_payload.txt', 'w');
	}
	if($result->num_rows > 0){
		...
		//-------- get the webhook data ------------
			...
			if(DEV_ENV){
				$sfd = fopen('shopify_orders_partially_fulfilled.txt', 'w');
			...
			if($result->num_rows > 0){
				...
				$query = "SELECT * FROM `emarsys_order_placeholders` WHERE `fkShopifyEventID` = ' " . ORDER_PARTIALLY_FULFILLED . "' AND `store_name` = '" . $shop_name . "'";
