...
if($verified == 1){
	if(DEV_ENV){
		$file = fopen('cart_update_paylooad.txt', 'w');
	}
	$query = "SELECT * FROM `emarsys_event_mapper` WHERE `fkShopifyEventID` = '" . CARTS_UPDATE . "' AND `store_name` = '" . trim($shop_name) . "'";
	...
	if($result->num_rows > 0){
		...
		//-------- get the webhook data ------------
			...
			if(DEV_ENV){
				$sfd = fopen('shopify_cart_update.txt', 'w');
			...
			if($result->num_rows > 0){
				...
				$query = "SELECT * FROM `emarsys_cart_placeholders` WHERE `fkShopifyEventID` = ' " . CARTS_UPDATE . "' AND `store_name` = '" . $shop_name . "'";
