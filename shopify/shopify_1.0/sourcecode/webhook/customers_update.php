...
// if($verified == 1){
	...
	if(DEV_ENV){
		$sfd = fopen('shopify_customer_update.txt', 'w');
		fwrite($sfd, $webhookContent);
		fclose($sfd);
	}
	
	//	TRIGGER CUSTOMER UPDATE EVENT START
	

	$query = "SELECT * FROM `emarsys_event_mapper` WHERE `fkShopifyEventID` = '" . CUSTOMER_UPDATE . "' AND `store_name` = '" . trim($hmac_shop) . "'";
	$result = $con->query($query);
	if(DEV_ENV){
		$file = fopen('customer_update_payload.txt', 'w');
	}
	if($result->num_rows > 0){
		...
		if(!empty($row['fkEventID'])){
			...
			if($result->num_rows > 0){
				...
				$query = "SELECT * FROM `emarsys_customer_placeholder` WHERE `fkShopifyEventID` = ' " . CUSTOMER_UPDATE . "' AND `store_name` = '" . $hmac_shop . "'";
				$result = $con->query($query);
				if($result->num_rows > 0){
	...
	//	TRIGGER CUSTOMER UPDATE EVENT END
   ...
   if (array_key_exists("14",$arr)) {
	...
	$file1 = fopen('a1_json_update.txt', 'w');
	...
	if ($shop_optin == 2) {
		...
		if($shopify_data['accepts_marketing']){
			...
			$sql123 = "SELECT * FROM emarsys_external_events where store_name='" . trim($shop) . "' AND eventName='Shopify_confirmation_email'";
             ...
             if ($result_contact_id123->num_rows > 0) {
                ...
                while($row_contact_id123 = $result_contact_id123->fetch_assoc()) {
                    ...
                    $db_external_event_id = $row_contact_id123["eventEmarsysID"];
                }
            }
    		...
    		$email_campaign = $emarsyClient->send('POST', 'event/'.$db_external_event_id.'/trigger', '{
  "key_id": "3",
