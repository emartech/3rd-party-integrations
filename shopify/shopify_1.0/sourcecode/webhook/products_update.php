...
if($verified == 1){
	...
	$query = "SELECT * FROM `emarsys_event_mapper` WHERE `fkShopifyEventID` = '" . PRODUCTS_UPDATE . "' AND `store_name` = '" . trim($shop_name) . "'";
	$result = $con->query($query);
	if(DEV_ENV){
		$file = fopen('products_update_paylooad.txt', 'w');
	}
	if($result->num_rows > 0){
		...
		//-------- get the webhook data ------------
			...
			if(DEV_ENV){
				$sfd = fopen('shopify_product_update.txt', 'w');
			...
			if($result->num_rows > 0){
				...
				$query = "SELECT * FROM `emarsys_product_placeholders` WHERE `fkShopifyEventID` = ' " . PRODUCTS_UPDATE . "' AND `store_name` = '" . $shop_name . "'";
