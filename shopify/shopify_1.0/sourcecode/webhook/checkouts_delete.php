...
if($verified == 1){
	...
	$query = "SELECT * FROM `emarsys_event_mapper` WHERE `fkShopifyEventID` = '" . CHECKOUT_DELETE . "' AND `store_name` = '" . trim($shop_name) . "'";
	$result = $con->query($query);
	if(DEV_ENV){
		$file = fopen('checkout_delete_paylooad.txt', 'w');
	}
	if($result->num_rows > 0){
		...
		//-------- get the webhook data ------------
			...
			if(DEV_ENV){
				$sfd = fopen('shopify_checkout_delete.txt', 'w');
			...
			if($result->num_rows > 0){
				...
				$query = "SELECT * FROM `emarsys_checkout_placeholders` WHERE `fkShopifyEventID` = ' " . CHECKOUT_DELETE . "' AND `store_name` = '" . $shop_name . "'";

				$result = $con->query($query);
				if($result->num_rows > 0){
					require_once 'payload/checkouts.php';
				}
			}
