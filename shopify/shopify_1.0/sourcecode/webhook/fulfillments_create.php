...
if($verified == 1){
	...
	$query = "SELECT * FROM `emarsys_event_mapper` WHERE `fkShopifyEventID` = '" . FULFILLMENT_CERATE . "' AND `store_name` = '" . trim($shop_name) . "'";
	$result = $con->query($query);
	if(DEV_ENV){
		$file = fopen('fulfuillments_create_paylooad.txt', 'w');
	}
	if($result->num_rows > 0){
		...
		//-------- get the webhook data ------------
			...
			if(DEV_ENV){
				$sfd = fopen('shopify_fulfillment_create.txt', 'w');
			...
			if($result->num_rows > 0){
				...
				$query = "SELECT * FROM `emarsys_fulfillments_placeholders` WHERE `fkShopifyEventID` = ' " . FULFILLMENT_CERATE . "' AND `store_name` = '" . $shop_name . "'";

				$result = $con->query($query);
				if($result->num_rows > 0){
					require_once 'payload/fulfillment.php';
				}
			}
